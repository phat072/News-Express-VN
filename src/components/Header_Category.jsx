        ...
        <div className='w-full'>
            <div className='bg-[#c80000] w-full text-white uppercase font-semibold relative'>
                <div className='px-8 flex justify-between items-center relative h-[48px]'>
                    ...
                    <div className='flex-wrap hidden lg:flex'>
                        <a className={`px-6 font-medium py-[13px] ${path === '/' ? 'bg-[#00000026]' : ''}`} href={'/'} >Home</a>
                        {
                            data.map((c) => <a key={c.id} className={`px-6 font-medium py-[13px] ${path === c.name ? 'bg-[#00000026]' : ''}`} href={'/'} >{c.name}</a>)
                        }
                    </div>
            ...
            </div>
            {
                cate_show && <div className='flex flex-wrap lg:hidden py-2 px-[30px]'>
                    <a className={`px-4 font-medium py-[5px] ${path === '/' ? 'bg-[#00000026]' : ''}`} href={'/'} >Home</a>
                    {
                        data.map((c) => <a key={c.id} className={`px-4 font-medium py-[5px] ${path === c.name ? 'bg-[#00000026]' : ''}`} href={'/'} >{c.name}</a>)
                    }
                </div>
            }
